...
public class ChangePendingCourseStateIT {
  ...
  void createAdminAndLogin() throws Exception {
    // Create APPROVED course state
    courseStateRepo.save(CourseState.builder()
        .name(courseStateProperties.getApproved())
        .build());

    // Create REJECTED course state
    courseStateRepo.save(CourseState.builder()
        .name(courseStateProperties.getRejected())
        .build());
  ...
  @Test
  void givenCourseIdAndIsApproved_whenGetPendingCourse_thenReturnOk() throws Exception {
    // given
    ...
    ResultActions resultActions = mockMvc
        .perform(put("/api/admins/pending-courses/" + courseId)
            .header(AUTHORIZATION, adminAccessToken)
            .param("isApproved", String.valueOf(isApproved)));

    // then
    resultActions.andExpect(status().isOk());
    assertThat(courseRepo.findById(courseId).get().getCourseState().getName())
        .isEqualTo(courseStateProperties.getApproved());
  }

  @Test
  void givenCourseIdAndIsRejected_whenGetPendingCourse_thenReturnOk() throws Exception {
    // given
    long courseId = createdCourseId;
    ...
    resultActions.andExpect(status().isOk());
    assertThat(courseRepo.findById(courseId).get().getCourseState().getName())
        .isEqualTo(courseStateProperties.getRejected());
  }
